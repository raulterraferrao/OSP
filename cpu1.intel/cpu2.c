...
void dispatch()
{
	...
	if (!fila_vazia()){ 					/* Verifica se a fila não esta vazia */
		...
		if(proximo_processo->last_dispatch != -1){	/* Verifica se o processo ja foi escalanado uma vez */
			if(proximo_processo->last_cpuburst < (*proximo_processo->hook)/2){ /* Caso da ultima vez o processo tenha utilizado menos que metade do quantum */
			...
			if(proximo_processo->last_cpuburst >= *proximo_processo->hook){ /* Verifica se o processo utilizou toda o quantum */
			...
			//fprintf(stderr, "\n\nProcesso %d , tempo desde o ultimo escalonamento %d, tempo de cpu %d, tempo acumulado %d",proximo_processo->pcb_id, get_clock() - proximo_processo->last_dispatch,proximo_processo->last_cpuburst,proximo_processo->accumulated_cpu);			
			//fprintf(stderr, "\nQuantum %d",*proximo_processo->hook);
